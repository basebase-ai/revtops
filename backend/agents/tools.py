...
- LOCAL_WRITE: run_sql_write, run_workflow, keep_notes, manage_memory, foreach, initiate_connector
...
async def _write_on_connector(
    ...
    if connector == "apps":
        ctx_apps: dict[str, Any] = context or {}
        if ctx_apps.get("conversation_id"):
...
async def _execute_linear_create(
    ...
    logger.info(
        "[Tools._execute_linear_create] Creating Linear issue for team_key=%s title=%s",
        team_key,
        title,
    )
...
async def _execute_linear_update(
    ...
    if raw_attachments is None:
        ...
        has_attachments = any(
            x is not None and str(x).strip() for x in raw_attachments
        )
    else:
        has_attachments = bool(str(raw_attachments).strip())
    ...
    logger.info(
        "[Tools._execute_linear_update] Updating Linear issue identifier=%s",
        issue_identifier,
    )
