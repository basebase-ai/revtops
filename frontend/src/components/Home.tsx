 ...
 * or the daily team digest (workstream map code kept but UI disabled).
 */

import { useCallback, useEffect, useState } from 'react';
import { fetchDailyDigests } from '../api/daily-digests';
...
import { DailyDigestGrid } from './DailyDigestGrid';
...
/** When true, the semantic workstream map is shown instead of daily digests. */
const SHOW_WORKSTREAM_MAP_HOME: boolean = false;
...
export function Home(): JSX.Element {
  ...
  /** Default digest date from API (yesterday PT); drives date navigation. */
  ...
  useEffect(() => {
    ...
    setDigestDate(null);
    if (SHOW_WORKSTREAM_MAP_HOME) {
      void fetchWorkstreamsData();
    }
    void (async () => {
      const d = await fetchDailyDigests();
      if (d.data?.digest_date) {
        setDigestDate(d.data.digest_date);
      } else {
        const dt: Date = new Date();
        dt.setUTCDate(dt.getUTCDate() - 1);
        setDigestDate(dt.toISOString().slice(0, 10));
      }
    })();
  ...
  // ---------------------------------------------------------------------------
  // Default: Daily digest (workstream map optional via SHOW_WORKSTREAM_MAP_HOME)
  // ---------------------------------------------------------------------------

  if (SHOW_WORKSTREAM_MAP_HOME && workstreamError && !workstreamData) {
    return (
      <div className="flex-1 flex items-center justify-center">
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-auto flex flex-col">
        ...
        {SHOW_WORKSTREAM_MAP_HOME ? (
          ...
          <div className="flex-1 min-h-[400px] flex items-center justify-center text-surface-400 gap-2">
            <svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
            </svg>
        ...
        {SHOW_WORKSTREAM_MAP_HOME && workstreamData?.computed_at ? (
