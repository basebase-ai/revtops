...
export function AdminPanel(): JSX.Element {
  ...
  const fetchWaitlist = useCallback(async (): Promise<void> => {
    ...
    try {
      const { data, error: requestError } = await apiRequest<{ entries: WaitlistEntry[]; total: number }>(
        `/waitlist/admin/list?status=${filter}`
      );

      if (requestError || !data) {
        setError(requestError?.includes('Global admin access required')
          ? 'Access denied. You need global_admin role.'
  ...
  const fetchUsers = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (requestError || !data) {
        setUsersError(requestError?.includes('Global admin access required')
          ? 'Access denied. You need global_admin role.'
  ...
  const fetchOrganizations = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (requestError || !data) {
        setOrgsError(requestError?.includes('Global admin access required')
          ? 'Access denied. You need global_admin role.'
  ...
  const handleInvite = async (targetUserId: string): Promise<void> => {
    ...
    try {
      const { error: inviteError } = await apiRequest<{ success: boolean; message: string; user_id: string }>(
